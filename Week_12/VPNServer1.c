...
//--------------------------- arp table && device && ip table set -----------------------------------
    ...
    memcpy(Device[0].mac_addr, "00:0c:29:de:bf:9b", 18);
    strcpy(Device[0].ip_addr, "10.0.0.1");
    Device[0].is_entrance = 0;
	strcpy(Device[1].interface, "eth1");
    memcpy(Device[1].mac_addr, "00:0c:29:de:bf:a5", 18);
    strcpy(Device[1].ip_addr, "192.168.0.2");
    Device[1].is_entrance = 1;
    device_index += 2;

	strcpy(route_info[0].destination, "10.0.0.0");//PC1
	...
	strcpy(route_info[1].destination, "192.168.0.0");//Router1
	...
	strcpy(route_info[2].destination, "10.0.1.0");//PC2
	strcpy(route_info[2].gateway, "192.168.0.1");//下一跳直接为网关地址
	strcpy(route_info[2].netmask, "255.255.255.0");
	strcpy(route_info[2].interface, "eth1");
    strcpy(route_info[2].vpn_ip, "172.0.0.2");
	route_item_index += 3;
