...
int ARP_MATCH(int* sock, unsigned char* buffer_rece_arp);
...
//------------------------------- arp table && device set ------------------------------------------
    ...
    memcpy(Device[0].mac_addr, "00:0c:29:c5:1c:c8", 18);
    device_index++;
//-------------------------------------------------------------------------------------------------

//------------------------------- open ------------------------------------------------------------
    int sock;
    if((sock=socket(PF_PACKET,SOCK_RAW,htons(ETH_P_ALL)))<0){//打开接收
...
//-----------------------------------------------------------------------------------------------
    if(arp_flag == 1){//ARP表中有该项，使用该项
        ...
        while(ARP_MATCH(sock, buffer_rece_arp) != 1){;}//等待ARP回复
//-------------------------- 匹配，添加表项 -------------------------------------------------------
